...
import com.kingdom.sdk.ioc.annotation.Inject;
import com.kingdom.sdk.ioc.annotation.InjectAssert;
import com.kingdom.sdk.ioc.annotation.InjectExtra;
import com.kingdom.sdk.ioc.annotation.InjectResource;
...
public class InjectUtil {
	...
	public static void injectFragment(Object obj,View view) {
		...
		if (fields != null && fields.length > 0) {
			for (Field field : fields) {
				...
				InjectView viewInject = field.getAnnotation(InjectView.class);
				if (viewInject != null) {
					indectFragmentView(obj, field,view);
				}
	 ...
	 * @param rootView
	 */
	public static void indectFragmentView(Object obj, Field field,View rootView) {
		...
		if (layout != 0) {
			...
			if (!TextUtils.isEmpty(inView)) {
				...
				view = rootView.findViewById(viewInject.id());
			}
		}
	...
	private static  boolean isCommonClazz(Class<?> clazz){
		...
		boolean isok=pkg.startsWith("com.kingdom.android")||pkg.startsWith(IocContainer.getShare().getApplicationContext().getPackageName());
		if(isok) {
			return true;
		}
		return false;
